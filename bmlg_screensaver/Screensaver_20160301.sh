...
startTime="1200"                    # Integer - Seconds
loginWindowIdleTime="1200"	# Integer - Seconds
justMain=""                     # Boolean
...
timeBeforeRequiringPassword="300"  # Integer - Seconds
#styleKey="Classic"	# String
#folderidentifier="/FOTL/screensaver"	# String
#foldername="FOTL Screensaver"	# String
#SelectedFolderPath="$folderidentifier"	# String
...
# Variables for the ~/Library/Preferences/com.apple.ScreenSaverPhotoChooser.ByHost

# Make sure the CustomFolderDict dictionary exists
        /usr/libexec/PlistBuddy -c "Add :CustomFolderDict dict" "$loggedInUserHome"/Library/Preferences/ByHost/com.apple.ScreenSaverPhotoChooser."$macUUID".plist

if [[ -n $folderidentifier ]]; then
        /usr/libexec/PlistBuddy -c "Add :CustomFolderDict:identifier string $folderidentifier" "$loggedInUserHome"/Library/Preferences/ByHost/com.apple.ScreenSaverPhotoChooser."$macUUID".plist
fi

if [[ -n $foldername ]]; then
        /usr/libexec/PlistBuddy -c "Add :CustomFolderDict:name string $foldername" "$loggedInUserHome"/Library/Preferences/ByHost/com.apple.ScreenSaverPhotoChooser."$macUUID".plist
fi

if [[ -n $SelectedFolderPath ]]; then
        /usr/libexec/PlistBuddy -c "Add :SelectedFolderPath string $SelectedFolderPath" "$loggedInUserHome"/Library/Preferences/ByHost/com.apple.ScreenSaverPhotoChooser."$macUUID".plist
fi

# Variables for the ~/Library/Preferences/com.apple.ScreenSaver.iLifeSlideShows.ByHost

if [[ -n $styleKey ]]; then
        /usr/libexec/PlistBuddy -c "Add :styleKey string $styleKey" "$loggedInUserHome"/Library/Preferences/ByHost/com.apple.ScreenSaver.iLifeSlideShows."$macUUID".plist
fi
...
#Kill cfprefsd so that the preferences will not get overwritten.
